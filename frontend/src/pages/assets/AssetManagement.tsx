...
const AssetManagement = () => {
  ...
  const [assignDialog, setAssignDialog] = useState<{
    ...
    employeeId: string;
    condition: string;
  }>({ open: false, asset: null, employeeId: '', condition: 'Good' });
  ...
  const handleAssign = async () => {
    if (!assignDialog.asset || !assignDialog.employeeId) {
      alert('Select employee');
      return;
    }
    setSaving(true);
    try {
      await api.post(`/assets/${assignDialog.asset._id}/assign`, {
        employeeId: assignDialog.employeeId,
        condition: assignDialog.condition,
      });
  ...
  const handleReturn = async (id: string) => {
    ...
    try {
      await api.post(`/assets/${id}/return`, { condition: 'Good' });
      fetchData();
    } catch (e: any) {
      alert(e.response?.data?.message || 'Failed to return');
    }
  };
                      ...
                      <TableCell align="right">
                        ...
                        {a.status === 'Assigned' && (
                          <Tooltip title="Return">
                            <IconButton size="small" onClick={() => handleReturn(a._id)}>
                              <Icon icon="material-symbols:keyboard-return" />
                            </IconButton>
                          </Tooltip>
                        )}
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleSave} disabled={saving}>
            {saving ? <CircularProgress size={18} /> : 'Add Asset'}
          </Button>
        </DialogActions>
      </Dialog>
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <FormControl fullWidth size="small">
              ...
              <Select
                value={assignDialog.employeeId}
                label="Employee *"
                onChange={(e) => setAssignDialog({ ...assignDialog, employeeId: e.target.value })}
              >
                {employees.map((e) => (
            ...
            <FormControl fullWidth size="small">
              ...
              <Select
                value={assignDialog.condition}
                label="Condition"
                onChange={(e) => setAssignDialog({ ...assignDialog, condition: e.target.value })}
              >
