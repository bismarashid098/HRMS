...
const DOC_TYPES = ['CNIC', 'Passport', 'Degree', 'Experience Letter', 'Contract', 'Offer Letter', 'Warning Letter', 'Policy', 'Other'];
...
const DocumentsPage = () => {
  ...
  const [form, setForm] = useState({ employee: '', title: '', type: 'Other', expiryDate: '', notes: '', isSharedWithEmployee: 'false' });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setOpen(false);
      setFile(null);
      setForm({ employee: '', title: '', type: 'Other', expiryDate: '', notes: '', isSharedWithEmployee: 'false' });
      fetchData();
    } catch (e: any) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        ...
        <Button variant="contained" startIcon={<Icon icon="material-symbols:upload" />} onClick={() => { setError(''); setOpen(true); }}>Upload Document</Button>
      </Stack>
        ...
        <CardContent>
          ...
          {loading ? <Box display="flex" justifyContent="center" py={4}><CircularProgress /></Box> : (
            <Table size="small">
              ...
              <TableBody>
                {docs.length === 0 ? <TableRow><TableCell colSpan={6} align="center">No documents found</TableCell></TableRow>
                  : docs.map((d) => (
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            ...
            <Button variant="outlined" component="label" startIcon={<Icon icon="material-symbols:upload" />}>
              {file ? file.name : 'Select File *'}
              <input type="file" hidden onChange={(e) => setFile(e.target.files?.[0] || null)} />
            </Button>
            <TextField label="Title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} fullWidth size="small" placeholder="Leave blank to use filename" />
            ...
            <TextField label="Expiry Date" type="date" value={form.expiryDate} onChange={(e) => setForm({ ...form, expiryDate: e.target.value })} size="small" InputLabelProps={{ shrink: true }} />
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleUpload} disabled={saving}>{saving ? <CircularProgress size={18} /> : 'Upload'}</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
