...
const Candidates = () => {
  ...
  const [statusDialog, setStatusDialog] = useState<{
    ...
    status: string;
    notes: string;
  }>({ open: false, candidate: null, status: '', notes: '' });
  ...
  const handleSave = async () => {
    ...
    if (!form.jobPosting) {
      setError('Job posting is required');
      return;
    }
    setSaving(true);
    try {
  ...
  const handleStatusUpdate = async () => {
    ...
    setSaving(true);
    try {
      await api.put(`/recruitment/candidates/${statusDialog.candidate._id}/status`, {
          ...
          <Stack spacing={2} mt={1}>
            ...
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                label="Job Posting *"
                onChange={(e) => setForm({ ...form, jobPosting: e.target.value })}
              >
                {jobs.map((j) => (
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <Typography variant="body2">
              Candidate: <b>{statusDialog.candidate?.name}</b>
            </Typography>
            <FormControl fullWidth size="small">
